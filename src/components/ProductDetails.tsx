...
export const ProductDetails = ({ product, onClose }: ProductDetailsProps) => {
  ...
  const primaryImage = product.images[0] || 'https://placehold.co/900x900?text=Beauty+Store';
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white w-full max-w-4xl h-[90vh] md:h-auto md:max-h-[90vh] rounded-t-[32px] md:rounded-[32px] overflow-hidden relative flex flex-col md:flex-row"
      >
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-10 w-10 h-10 rounded-full bg-white/80 backdrop-blur shadow-md flex items-center justify-center text-gray-800 hover:bg-white transition-colors"
        >
          <X size={20} />
        </button>

        <div className="w-full md:w-1/2 h-1/2 md:h-auto bg-gray-50 flex flex-col">
          <img src={primaryImage} alt={product.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
          {product.images.length > 1 && (
            <div className="grid grid-cols-4 gap-2 p-2 bg-white border-t border-gray-100">
              {product.images.slice(0, 4).map((image) => (
                <img key={image} src={image} alt={product.name} className="w-full aspect-square object-cover rounded" />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto">
          ...
          <h2 className="font-serif text-3xl md:text-4xl text-gray-900 mb-2">{product.name}</h2>
          <p className="text-beauty-pink-dark font-bold text-2xl mb-6">{product.price}</p>
